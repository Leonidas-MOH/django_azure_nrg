...
#######################
from .models import SpotDetNew
ModelClassName = SpotDetNew
...
Model_Fields = ['metric_id','country_id','curvesnameprice','active','daybefore','dayafter','insertminutes']
Table_Sequence = ['metric_id','country_id','detail','detaile','...']
Table_Exclude = ['id','tableto']
Rows_Per_Page = 25
...
class CurrentTable(ExportMixin, tables.Table):
    ...
    detaile = tables.LinkColumn(PathStart+'edit', args=[A('pk')], orderable=False, empty_values=[''])    
    class Meta:
        model = ModelClassName
        row_attrs = {
    ...
    def render_detail(self, record):
        ...
        return mark_safe('<a href=' + rev + f'><span style="color:green">{DefComName}</span></a>')
    def render_detaile(self, record):
        rev = reverse(PathStart+'edit', kwargs={'pk': str(record.id)})
        return mark_safe('<a href=' + rev + f'><span style="color:red">Μεταβολή</span></a>')
