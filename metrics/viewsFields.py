...
#######################
from .models import FieldParam

ModelClassName = FieldParam
...
Model_Fields = ['table','field_name','field_description']
Table_Sequence = ['table','detail','detailed','field_name','field_description','...']
Table_Exclude = ['id']
Rows_Per_Page = 25
...
class CurrentTable(ExportMixin, tables.Table):
    ...
    detailed = tables.LinkColumn(PathStart+'view', args=[A('pk')], orderable=False, empty_values=[''])    
    class Meta:
        model = ModelClassName
        row_attrs = {
...
def DetailFiltered(request):
    ...
    if TableExport.is_valid_format(export_format):
        exporter = TableExport(export_format, table ,exclude_columns = ('detail','detailsed', ))
